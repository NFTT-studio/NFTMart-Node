...
pub(crate) fn network_id_from_bytes(
	...
	ensure!(encoded_bytes.len() > 0, gasometer.revert("Junctions cannot be empty"));
	...
	match network_selector[0] {
		0 => Ok(NetworkId::Any),
		1 => Ok(NetworkId::Named(encoded_network_id.read_till_end(gasometer)?.to_vec())),
...
impl EvmData for Junction {
	fn read(reader: &mut EvmDataReader, gasometer: &mut Gasometer) -> EvmResult<Self> {
		...
		ensure!(junction_bytes.len() > 0, gasometer.revert("Junctions cannot be empty"));
		...
		match enum_selector[0] {
			...
			4 => Ok(Junction::PalletInstance(encoded_junction.read_raw_bytes(gasometer, 1)?[0])),
			5 => {
			...
			6 => Ok(Junction::GeneralKey(encoded_junction.read_till_end(gasometer)?.to_vec())),
